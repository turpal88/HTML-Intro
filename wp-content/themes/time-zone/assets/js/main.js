(function ($) {
  ...
  const sortItems = $(".nice-select .list li");
  sortItems.on("click", function (event) {
    window.location.href =
      "/time-zone/shop?orderby=" + $(event.target).attr("data-value");
  });
})(jQuery);
function inputIncrement(input) {
  input.value = parseInt(input.value) + 1;
}
function inputDecrement(input) {
  if (input.value != 0) {
    input.value = parseInt(input.value) - 1;
  }
}
